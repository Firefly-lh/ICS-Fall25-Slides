...
4.57 A. 主要关注当中的指针运算部分(回忆下**自动伸缩**这一知识点)
...
4.58 如下实现了简单的比较交换逻辑, 注释部分标注了对应汇编代码的理解
...
4.59 由于只允许使用一次条件传送, 你需要通过尽量少传送的方法交换a和b的值, 这里采用的解决方案是先求较小值, 而后利用两数之和还原最大值的方式. ([这个网站](https://dreamanddead.github.io/CSAPP-3e-Solutions/chapter4/4.49/)记录了另一个可行的方法, 用到了三异或交换的巧妙性质)
